...
class SceneCamera : public ControlledCamera,
                    public ShadowCascadesCamera<Cascades> {
public:
  SceneCamera(float fov, float ratio)
      : ShadowCascadesCamera<Cascades>(fov, ratio){};
...
public:
  ...
  SceneProjector(uint32_t width, uint32_t height, std::string const &title)
      : Window(width, height, title),
        m_camera(60.0f, (float)width / (float)height) {}
