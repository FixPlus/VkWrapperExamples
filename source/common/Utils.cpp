...
void requestQueues(vkw::PhysicalDevice &physicalDevice, bool wantTransfer,
                   bool wantCompute) {
  ...
  auto graphicsFamily = std::find_if(
      families.begin(), families.end(),
      [](vkw::QueueFamily const &family) { return family.graphics(); });
  ...
  if (wantTransfer) {
    auto transferFamily = std::find_if(
        families.begin(), families.end(),
        [](vkw::QueueFamily const &family) { return family.transfer(); });

    if (transferFamily->index() == graphicsFamily->index()) {
      transferFamily++;
      transferFamily = std::find_if(
          transferFamily, families.end(),
          [](vkw::QueueFamily const &family) { return family.transfer(); });
    }
  ...
  if (wantCompute) {
    auto computeFamily = std::find_if(
        families.begin(), families.end(),
        [](vkw::QueueFamily const &family) { return family.compute(); });

    if (computeFamily->index() == graphicsFamily->index()) {
      computeFamily++;
      computeFamily = std::find_if(
          computeFamily, families.end(),
          [](vkw::QueueFamily const &family) { return family.compute(); });
    }
