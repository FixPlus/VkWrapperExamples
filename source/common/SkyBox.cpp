...
SkyBox::SkyBox(vkw::Device &device, vkw::RenderPass const &pass,
               uint32_t subpass,
               RenderEngine::ShaderLoaderInterface &shaderLoader)
    : m_device(device),
      m_geometry_layout(device, shaderLoader,
                        RenderEngine::GeometryLayout::CreateInfo{
                            nullptr, vkw::InputAssemblyStateCreateInfo{},
                            RenderEngine::SubstageDescription{"skybox"}, 1}),
      m_projection_layout(
          device, shaderLoader,
          RenderEngine::SubstageDescription{.shaderSubstageName = "skybox"}, 1),
      m_material_layout(device, shaderLoader,
                        RenderEngine::MaterialLayout::CreateInfo{
                            RenderEngine::SubstageDescription{"skybox"},
                            vkw::RasterizationStateCreateInfo{},
                            std::optional<vkw::DepthTestStateCreateInfo>{}, 1}),
      m_lighting_layout(
          device, shaderLoader,
          RenderEngine::LightingLayout::CreateInfo{
              RenderEngine::SubstageDescription{"flat"}, pass, subpass},
          1),
      m_geometry(m_geometry_layout), m_projection(m_projection_layout),
...
SkyBoxSettings::SkyBoxSettings(TestApp::GUIFrontEnd &gui, SkyBox &skybox,
                               const std::string &title)
    : GUIWindow(gui, TestApp::WindowSettings{.title = title}),
      m_skybox(skybox) {}

void SkyBoxSettings::onGui() {
  if (ImGui::CollapsingHeader("Sun")) {
    ImGui::ColorEdit4("color", &m_skybox.get().sun.color.x);
    ImGui::SliderFloat("irradiance", &m_skybox.get().sun.params.z, 0.1f,
                       1000.0f);
    ImGui::SliderFloat("lon", &m_skybox.get().sun.params.x, 0.0f,
                       glm::pi<float>() * 2.0f);
    ImGui::SliderFloat("lat", &m_skybox.get().sun.params.y, 0.0f,
                       glm::pi<float>());
  }
  if (ImGui::CollapsingHeader("Atmosphere")) {
    m_need_recompute_outscatter =
        ImGui::SliderFloat4("K", &m_skybox.get().atmosphere.K.x, 0.0f, 1.0f) ||
        m_need_recompute_outscatter;
    m_need_recompute_outscatter =
        ImGui::SliderFloat("H0", &m_skybox.get().atmosphere.params.z, 0.0f,
                           1.0f) ||
        m_need_recompute_outscatter;
    m_need_recompute_outscatter =
        ImGui::SliderFloat("Atmosphere height",
                           &m_skybox.get().atmosphere.params.y, 0.0f,
                           10000.0f) ||
        m_need_recompute_outscatter;
    m_need_recompute_outscatter =
        ImGui::SliderFloat("g", &m_skybox.get().atmosphere.params.w, -1.0f,
                           0.0f) ||
        m_need_recompute_outscatter;
    m_need_recompute_outscatter =
        ImGui::SliderInt("samples", &m_skybox.get().atmosphere.samples, 5,
                         50) ||
        m_need_recompute_outscatter;
  }
}

SkyBox::OutScatterTexture::OutScatterTexture(
    vkw::Device &device, RenderEngine::ShaderLoaderInterface &shaderLoader,
    vkw::UniformBuffer<Atmosphere> const &atmo, uint32_t psiRate,
    uint32_t heightRate)
    : vkw::Image<vkw::COLOR,
