...
public:
  GraphicsRecordingState(vkw::CommandBuffer &command,
                         GraphicsPipelinePool &pool)
      : m_commandBuffer(command), m_pool(pool) {}
  ...
  template <typename T>
  void pushConstants(T constant, VkShaderStageFlagBits shaderStage,
                     uint32_t offset) {
    m_commandBuffer.get().pushConstants(m_current_layout(), shaderStage, offset,
                                        constant);
  }
