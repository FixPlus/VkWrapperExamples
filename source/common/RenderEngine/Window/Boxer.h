...
public:
  ...
  static Selection show(std::string_view const &message,
                        std::string_view const &title, Style style,
                        Buttons buttons);

  static inline Selection show(std::string_view const &message,
                               std::string_view const &title, Style style) {
    return show(message, title, style, DEFAULT_BUTTONS);
  }

  static inline Selection show(std::string_view const &message,
                               std::string_view const &title, Buttons buttons) {
