...
GUIBackend::GUIBackend(vkw::Device &device, vkw::RenderPass &pass,
                       uint32_t subpass,
                       RenderEngine::ShaderLoaderInterface &shaderLoader,
                       RenderEngine::TextureLoader textureLoader)
    : m_device(device), m_sampler(m_sampler_init(device)),
          ...
          RenderEngine::GeometryLayout::CreateInfo{
              .vertexInputState =
                  std::make_unique<vkw::VertexInputStateCreateInfo<
                      vkw::per_vertex<GUIBackend::GUIVertex, 0>>>(),
              .substageDescription =
                  RenderEngine::SubstageDescription{.shaderSubstageName = "ui"},
              .maxGeometries = 1}),
      m_geometry(m_geometryLayout),
      m_projectionLayout(
          device, shaderLoader,
          RenderEngine::SubstageDescription{.shaderSubstageName = "flat"}, 1),
      m_projection(m_projectionLayout),
      m_materialLayout(device, shaderLoader,
                       RenderEngine::MaterialLayout::CreateInfo{
                           RenderEngine::SubstageDescription{"ui"},
                           vkw::RasterizationStateCreateInfo{},
                           std::optional<vkw::DepthTestStateCreateInfo>{},
                           1000}),
      m_lightingLayout(device, shaderLoader,
                       RenderEngine::LightingLayout::CreateInfo{
                           RenderEngine::SubstageDescription{"flat"},
                           pass,
                           subpass,
                           {{getUIBlendState(), 0}}},
                       1),
...
void GUIBackend::m_updateFontTexture(ImFontAtlas *atlas) {
  ...
  auto fontTexture = m_font_loader.loadTexture(fontData, texWidth, texHeight);
  ...
  auto &newTexture =
      m_font_textures.emplace(nullptr, std::move(fontTexture)).first->second;

  auto *key = m_views_storage
                  .emplace_back(std::make_unique<TextureView>(
                      m_device, newTexture, fontTexture.format()))
                  .get();
  ...
  m_materials.emplace(key,
                      Material{m_device, m_materialLayout, *key, m_sampler});
  atlas->TexID = key;
}
...
vkw::VertexBuffer<GUIBackend::GUIVertex>
GUIBackend::m_create_vertex_buffer(vkw::Device &device, uint32_t size) {
...
vkw::IndexBuffer<VK_INDEX_TYPE_UINT16>
GUIBackend::m_create_index_buffer(vkw::Device &device, uint32_t size) {
  VmaAllocationCreateInfo createInfo{};
  createInfo.usage = VMA_MEMORY_USAGE_CPU_TO_GPU;
  createInfo.requiredFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
...
void GUIBackend::draw(RenderEngine::GraphicsRecordingState &recorder) {
  ...
  for (int32_t i = 0; i < imDrawData->CmdListsCount; i++) {
    ...
    for (int32_t j = 0; j < cmd_list->CmdBuffer.Size; j++) {
      ...
      recorder.commands().drawIndexed(pcmd->ElemCount, 1, indexOffset,
                                      vertexOffset, 0);
...
void WindowIO::mouseMove(double xpos, double ypos, double xdelta,
                         double ydelta) {
  ImGui::SetCurrentContext(m_context);
  auto &io = ImGui::GetIO();
  if (cursorDisabled()) {
...
GUIBackend::Material::Material(vkw::Device &device,
                               RenderEngine::MaterialLayout &layout,
                               TextureView const &texture,
                               const vkw::Sampler &sampler)
    : RenderEngine::Material(layout) {
  set().write(0, texture, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, sampler);
}

GUIWindow::GUIWindow(GUIFrontEnd &parent, WindowSettings settings)
    : m_parent(parent), m_settings(std::move(settings)) {
  m_compileFlags();
  m_parent.get().m_windows.emplace(this);
}

GUIWindow::~GUIWindow() { m_parent.get().m_windows.erase(this); }

GUIWindow::GUIWindow(GUIWindow &&another) noexcept
    : GUIWindow(another.m_parent, another.m_settings) {
...
GUIFrontEnd::GUIFrontEnd(GUIFrontEnd &&another) noexcept
    : m_windows(std::move(another.m_windows)) {
