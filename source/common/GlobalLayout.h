...
public:
  GlobalLayout(vkw::Device &device,
  ...
  void bind(RenderEngine::GraphicsRecordingState &state,
            bool useSimpleLighting = false) const {
    state.setProjection(m_camera_projection);
    state.setLighting(
        useSimpleLighting
            ? static_cast<RenderEngine::Lighting const &>(m_simple_light)
            : m_light);
  }

  void update() { m_camera_projection.update(m_camera); }

  TestApp::Camera const &camera() const { return m_camera.get(); }
...
private:
  ...
  struct CameraProjection : public RenderEngine::Projection {
    ...
    CameraProjection(vkw::Device &device,
                     RenderEngine::ProjectionLayout &layout);
  ...
  struct Light : public RenderEngine::Lighting {
    ...
    Light(vkw::Device &device, RenderEngine::LightingLayout &layout,
          TestApp::ShadowRenderPass &shadowPass, const SkyBox &skyBox);
  ...
  struct SimpleLight : public RenderEngine::Lighting {
    SimpleLight(vkw::Device &device, RenderEngine::LightingLayout &layout,
                const SkyBox &skyBox);
  } m_simple_light;
...
public:
  ...
  bool useSimpleLighting() const { return m_use_simple_lighting; }
