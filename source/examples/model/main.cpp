...
public:
  ModelTransform(GUIFrontEnd &gui)
      : GUIWindow(gui, WindowSettings{.title = "Model transform"}) {}

  std::function<void(void)> gui_impl = []() {};

protected:
  void onGui() override { gui_impl(); }
};
std::optional<std::filesystem::path>
modelPath(std::filesystem::path const &modelDir) {
  auto binary_path = modelDir / L"glTF-Binary";
  if (exists(binary_path)) {
    auto dirIt = std::filesystem::directory_iterator(binary_path);
    auto foundGLB = std::ranges::find_if(
        dirIt, [](auto &file) { return file.path().extension() == L".glb"; });
  ...
  if (exists(json_path)) {
    ...
    auto foundGLTF = std::ranges::find_if(
        dirIt, [](auto &file) { return file.path().extension() == L".gltf"; });
...
listAvailableModels(std::filesystem::path const &root) {
  if (!exists(root))
    return {};
...
std::unique_ptr<GLTFModel>
tryLoad(vkw::Device &renderer,
        RenderEngine::ShaderLoaderInterface &shaderLoader,
        DefaultTexturePool &pool, std::filesystem::path const &path) {
  ...
  try {
    ...
    RenderEngine::Boxer::show(e.what(), ss.str(),
...
public:
  ModelApp()
      : CommonApp(AppCreateInfo{true, "Model"}),
        shadowPass(device(), shaderLoader()),
        skybox(device(), onScreenPass(), 0, shaderLoader()),
        globalState{device(),          shaderLoader(), onScreenPass(), 0,
                    window().camera(), shadowPass,     skybox},
        skyboxSettings(gui(), skybox, "Sky box"), defaultTextures(device()),
        modelPipelinePool(device(), shaderLoader()), modelTransform(gui()) {
    modelList = listAvailableModels(EXAMPLE_GLTF_PATH);

    std::transform(modelList.begin(), modelList.end(),
                   std::back_inserter(modelListString),
                   [](std::filesystem::path const &path) {
                     return path.filename().string();
                   });
    std::transform(modelListString.begin(), modelListString.end(),
                   std::back_inserter(modelListCstr),
                   [](std::string const &name) { return name.c_str(); });
    if (modelList.empty()) {
      std::stringstream ss;
    ...
    model = std::make_unique<GLTFModel>(device(), shaderLoader(),
                                        defaultTextures, modelList.front());
    ...
    modelTransform.gui_impl = [this, loadedModel]() {
      ...
      bool upd = false;
      upd =
          ImGui::SliderFloat("rot x", &instance->rotation.x, -180.0f, 180.0f) |
          upd;
      upd =
          ImGui::SliderFloat("rot y", &instance->rotation.y, -180.0f, 180.0f) |
          upd;
      upd =
          ImGui::SliderFloat("rot z", &instance->rotation.z, -180.0f, 180.0f) |
          upd;
      upd =
          ImGui::SliderFloat("scale x", &instance->scale.x, 0.1f, 10.0f) | upd;
      upd =
          ImGui::SliderFloat("scale y", &instance->scale.y, 0.1f, 10.0f) | upd;
      upd =
          ImGui::SliderFloat("scale z", &instance->scale.z, 0.1f, 10.0f) | upd;

      if (upd)
        instance->update();

      auto oldSelect = current_model;
      if (ImGui::Combo("models", &current_model, modelListCstr.data(),
                       modelListCstr.size())) {
        {
        ...
        auto expectModel = tryLoad(device(), shaderLoader(), defaultTextures,
                                   modelList.at(current_model));
        if (!expectModel) {
        ...
        instance =
            std::make_unique<GLTFModelInstance>(model->createNewInstance());
        instance->update();
      }
      ...
      if (ImGui::SliderFloat("Far clip", &farClip,
                             window().camera().nearPlane(),
                             window().camera().farPlane())) {
...
protected:
  ...
  void onMainPass(vkw::PrimaryCommandBuffer &buffer,
    ...
    RenderEngine::GraphicsRecordingState localRecorder{buffer,
                                                       modelPipelinePool};
...
int main() { return ErrorCallbackWrapper<decltype(&runModel)>::run(&runModel); }
