...
TestApp::Fractal::Fractal(vkw::Device &device, vkw::RenderPass &pass,
                          uint32_t subpass,
    ...
    : m_geom_layout(
          ...
          RenderEngine::GeometryLayout::CreateInfo{
              ...
              .substageDescription = {.shaderSubstageName = "skybox"}}),
      m_geometry(m_geom_layout),
      m_projection_layout(
          device, shaderLoader,
          RenderEngine::SubstageDescription{.shaderSubstageName = "skybox"}, 1),
      m_projection(m_projection_layout),
      m_material_layout(
          device, shaderLoader,
          RenderEngine::MaterialLayout::CreateInfo{
              .substageDescription = {.shaderSubstageName = "fractal"},
              .maxMaterials = 1}),
      m_material(m_material_layout, device, textureLoader),
      m_offscreen_lighting_layout(
          device, shaderLoader,
          RenderEngine::LightingLayout::CreateInfo{
              .substageDescription = {.shaderSubstageName = "colorDepth"},
              .pass = m_offscreenPass,
              .subpass = 0},
          1),
      m_offscreen_lighting(m_offscreen_lighting_layout),
      m_offscreenPass(device),
      m_offscreenBuffer(std::make_unique<OffscreenBuffer>(
          device, m_offscreenPass, texWidth, texHeight)),
      m_device(device),
      m_filter_layout(
          device, shaderLoader,
          RenderEngine::MaterialLayout::CreateInfo{
              .substageDescription = {.shaderSubstageName = "depthFilter"},
              .maxMaterials = 1}),
      m_filter_material(m_filter_layout, device, *m_offscreenBuffer),
      m_lighting_layout(
          device, shaderLoader,
          RenderEngine::LightingLayout::CreateInfo{
              .substageDescription = {.shaderSubstageName = "flat"},
              .pass = pass,
              .subpass = subpass},
          1),
      m_lighting(m_lighting_layout) {}

void TestApp::Fractal::drawOffscreen(vkw::PrimaryCommandBuffer &buffer,
  ...
  buffer.beginRenderPass(m_offscreenPass, *m_offscreenBuffer, renderArea, false,
...
void TestApp::Fractal::resizeOffscreenBuffer(uint32_t width, uint32_t height) {
  m_filter_material.removeTargetViews();
  m_offscreenBuffer = std::make_unique<OffscreenBuffer>(
      m_device.get(), m_offscreenPass, width, height);
  m_filter_material.updateTargetViews(*m_offscreenBuffer);
  m_filter_material.rewriteOffscreenTextures(*m_offscreenBuffer,
                                             m_device.get());
}
...
TestApp::Fractal::FractalMaterial::FractalMaterial(
    RenderEngine::MaterialLayout &parent, vkw::Device &device,
    RenderEngine::TextureLoader &textureLoader)
    : RenderEngine::Material(parent),
      ...
      m_device(device), m_texture_view(device, m_texture, m_texture.format(),
                                       0u, m_texture.mipLevels()) {
  ...
  set().write(1, m_texture_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
              m_sampler);
  set().write(0, m_buffer);
}

void TestApp::Fractal::FractalMaterial::update(
    const TestApp::Fractal::UBO &ubo) {
  *m_mapped = ubo;
  m_buffer.flush();
}
...
void TestApp::Fractal::FractalMaterial::switchSamplerMode() {
  ...
  set().write(1, m_texture_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
              m_sampler_mode ? m_sampler_with_mips : m_sampler);
}

TestApp::FractalSettings::FractalSettings(TestApp::GUIFrontEnd &gui,
                                          TestApp::Fractal &fractal)
    : GUIWindow(gui, WindowSettings{.title = "Fractal", .autoSize = true}),
      m_fractal(fractal) {}

void TestApp::FractalSettings::onGui() {
  ImGui::SliderFloat("Mutate", &m_fractal.get().ubo.params.x, 0.0f, 2.0f);
  ImGui::SliderFloat3("Light Pos", &m_fractal.get().ubo.lightPos.x, -1.0f,
                      1.0f);
...
TestApp::Fractal::RenderPassAttachments::RenderPassAttachments(
    vkw::Device &device)
    : m_attachments(
          {vkw::AttachmentDescription{
...
TestApp::Fractal::RenderPass::RenderPass(vkw::Device &device)
    : RenderPassAttachments(device), vkw::RenderPass(
                                         device, m_compileRenderPassInfo()) {}

vkw::RenderPassCreateInfo
TestApp::Fractal::RenderPass::m_compileRenderPassInfo() {
  ...
  subpassDescription.addColorAttachment(
      depthAttachment(), VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
  ...
  return vkw::RenderPassCreateInfo(
      {std::array<std::reference_wrapper<vkw::AttachmentDescription const>, 2>{
           colorAttachment(), depthAttachment()},
       {subpassDescription},
       {inputDependency, outputDependency}});
}

TestApp::Fractal::OffscreenBufferImages::OffscreenBufferImages(
    vkw::Device &device, uint32_t width, uint32_t height)
    : m_colorTarget(
          device.getAllocator(),
      ...
      m_depthTarget(
          ...
          VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT),
      m_targetViews({vkw::ImageView<vkw::COLOR, vkw::V2DA>{
                         device, m_colorTarget, m_colorTarget.format()},
                     {device, m_depthTarget, m_depthTarget.format()}}),
      m_targetRefViews({&m_targetViews.at(0), &m_targetViews.at(1)}) {

  doTransitLayout(m_colorTarget, device, VK_IMAGE_LAYOUT_UNDEFINED,
                  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  doTransitLayout(m_depthTarget, device, VK_IMAGE_LAYOUT_UNDEFINED,
                  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
}

TestApp::Fractal::OffscreenBuffer::OffscreenBuffer(
    vkw::Device &device, TestApp::Fractal::RenderPass &pass, uint32_t width,
    uint32_t height)
    : OffscreenBufferImages(device, width, height),
      vkw::FrameBuffer(device, pass, VkExtent2D{width, height},
                       {targetViews().begin(), targetViews().end()}) {}

TestApp::Fractal::FilterMaterial::FilterMaterial(
    RenderEngine::MaterialLayout &parent, vkw::Device &device,
    TestApp::Fractal::OffscreenBuffer &offscreenBuffer)
    : RenderEngine::Material(parent),
      m_buffer(device,
               VmaAllocationCreateInfo{
                   ...
                   .requiredFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT}),
      m_sampler(TestApp::createDefaultSampler(device)),
      m_colorTargetView(std::make_unique<vkw::ImageView<vkw::COLOR, vkw::V2D>>(
          device, offscreenBuffer.colorTarget(),
          offscreenBuffer.colorTarget().format())),
      m_depthTargetView(std::make_unique<vkw::ImageView<vkw::COLOR, vkw::V2D>>(
          device, offscreenBuffer.depthTarget(),
          offscreenBuffer.depthTarget().format())),
      m_device(device) {
...
void TestApp::Fractal::FilterMaterial::update(
    const TestApp::Fractal::FilterUBO &ubo) {
  *m_mapped = ubo;
  m_buffer.flush();
}

void TestApp::Fractal::FilterMaterial::rewriteOffscreenTextures(
    TestApp::Fractal::OffscreenBuffer &offscreenBuffer, vkw::Device &device) {
  VkComponentMapping mapping{};
  mapping.r = VK_COMPONENT_SWIZZLE_IDENTITY;
  mapping.g = VK_COMPONENT_SWIZZLE_IDENTITY;
  mapping.b = VK_COMPONENT_SWIZZLE_IDENTITY;
  ...
  set().write(1, *m_colorTargetView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
              m_sampler);
  set().write(2, *m_depthTargetView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
              m_sampler);
}

void TestApp::Fractal::FilterMaterial::updateTargetViews(
    TestApp::Fractal::OffscreenBuffer &offscreenBuffer) {
  m_colorTargetView = std::make_unique<vkw::ImageView<vkw::COLOR, vkw::V2D>>(
      m_device, offscreenBuffer.colorTarget(),
      offscreenBuffer.colorTarget().format());
  m_depthTargetView = std::make_unique<vkw::ImageView<vkw::COLOR, vkw::V2D>>(
      m_device, offscreenBuffer.depthTarget(),
      offscreenBuffer.depthTarget().format());
}

void TestApp::Fractal::FilterMaterial::removeTargetViews() {
