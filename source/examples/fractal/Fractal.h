...
public:
  ...
  Fractal(vkw::Device &device, vkw::RenderPass &pass, uint32_t subpass,
          RenderEngine::ShaderLoaderInterface &shaderLoader,
          RenderEngine::TextureLoader &textureLoader, uint32_t texWidth,
          uint32_t texHeight);

  void update(TestApp::CameraPerspective const &camera);

  void drawOffscreen(vkw::PrimaryCommandBuffer &buffer,
                     RenderEngine::GraphicsPipelinePool &pool);

  void draw(RenderEngine::GraphicsRecordingState &recorder);

  void switchSamplerMode() { m_material.switchSamplerMode(); }
...
private:
  ...
  public:
    ...
    ColorImage2D &colorTarget() { return m_colorTarget; };

    ColorImage2D &depthTarget() { return m_depthTarget; }
  ...
  class OffscreenBuffer : public OffscreenBufferImages,
                          public vkw::FrameBuffer {
  public:
    OffscreenBuffer(vkw::Device &device, RenderPass &pass, uint32_t width,
                    uint32_t height);
  };
  ...
  public:
    FractalMaterial(RenderEngine::MaterialLayout &parent, vkw::Device &device,
  ...
  public:
    FilterMaterial(RenderEngine::MaterialLayout &parent, vkw::Device &device,
    ...
    void rewriteOffscreenTextures(OffscreenBuffer &offscreenBuffer,
                                  vkw::Device &device);
