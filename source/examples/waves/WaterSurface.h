...
class FFT {
public:
  explicit FFT(vkw::Device &device,
               RenderEngine::ShaderLoaderInterface &shaderLoader);

  void ftt(vkw::CommandBuffer &buffer, Complex2DTexture const &input,
           Complex2DTexture const &output);

private:
  class PermuteRow : public RenderEngine::Compute {
  public:
    PermuteRow(RenderEngine::ComputeLayout &layout,
               Complex2DTexture const &input, Complex2DTexture const &output);
  };
  ...
  public:
    PermuteColumn(RenderEngine::ComputeLayout &layout,
                  Complex2DTexture const &io);
  };
  ...
  std::map<std::pair<Complex2DTexture const *, Complex2DTexture const *>,
...
public:
  WaveSurfaceTexture(vkw::Device &device,
                     RenderEngine::ShaderLoaderInterface &shaderLoader,
                     uint32_t baseCascadeSize, uint32_t cascades = 1);

  vkw::BasicImage<vkw::COLOR, vkw::I2D, vkw::ARRAY> &
  cascade(uint32_t cascadeIndex) {
  ...
  size_t cascadesCount() const { return m_cascades.size(); }
  ...
  void releaseOwnershipTo(vkw::CommandBuffer &buffer,
    ...
    for (auto &cascade : m_cascades) {
      ...
      buffer.imageMemoryBarrier(incomingStageMask,
  ...
  void releaseOwnership(vkw::CommandBuffer &buffer, uint32_t acquireFamilyIndex,
    ...
    for (auto &cascade : m_cascades) {
      ...
      buffer.imageMemoryBarrier(VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                acquireStageMask, {&transitLayout1, 1});
    }
  }
...
private:
  ...
  class WaveSurfaceTextureCascade
      : public WaveSurfaceTextureCascadeImageHandler {
  public:
    WaveSurfaceTextureCascade(
    ...
    GlobalParams &params() { return m_spectrum_textures.globalParameters; }

  private:
    class SpectrumTextures
        : public vkw::Image<vkw::COLOR, vkw::I2D, vkw::ARRAY>,
          public TestApp::PrecomputeImage {
    public:
      SpectrumTextures(
          TestApp::PrecomputeImageLayout &spectrumPrecomputeLayout,
          vkw::UniformBuffer<SpectrumParameters> const &spectrumParams,
    ...
    public:
      ...
      vkw::ImageView<vkw::COLOR, vkw::V2D> const &
      displacementView(unsigned i) const {
    ...
    public:
      FinalTextureCombiner(RenderEngine::ComputeLayout &layout,
                           ...
                           WaveSurfaceTextureCascade &final,
                           vkw::Device &device);
    } m_texture_combiner;
  ...
  public:
    DynamicSpectrumTextures &spectrum() { return m_dynamic_spectrum; }
  };
...
protected:
  ...
  std::pair<float, float> heightBounds() const override {
    return {-5.0f, 5.0f};
  };
};
...
public:
  WaterMaterial(vkw::Device &device,
                RenderEngine::ShaderLoaderInterface &shaderLoader,
                WaveSurfaceTexture &texture, bool wireframe = false);
...
private:
  ...
  private:
    std::vector<std::pair<vkw::ImageView<vkw::COLOR, vkw::V2D>,
                          vkw::ImageView<vkw::COLOR, vkw::V2D>>>
        m_derivTurbViews;
...
public:
  WaveSettings(
      TestApp::GUIFrontEnd &gui, WaterSurface &water,
      WaveSurfaceTexture &texture,
  ...
  void resetUpdateSpectrum() { m_need_update_static_spectrum = false; }
