...
TestApp::PrecomputeImageLayout::PrecomputeImageLayout(
    ...
    : RenderEngine::ComputeLayout(device, shaderLoader, stageDescription, 10),
      m_xGroupSize(xGroupSize), m_yGroupSize(yGroupSize) {}

TestApp::PrecomputeImage::PrecomputeImage(
    vkw::Device &device, TestApp::PrecomputeImageLayout &parent,
    vkw::BasicImage<vkw::COLOR, vkw::I2D, vkw::ARRAY> &image)
    : RenderEngine::Compute(parent), m_image(image),
      m_imageView(device, image, image.format(), 0u, image.layers()),
      m_cached_group_size(image.width() / parent.xGroupSize(),
                          image.height() / parent.yGroupSize()) {

  set().writeStorageImage(0, m_imageView, VK_IMAGE_LAYOUT_GENERAL);
}

void TestApp::PrecomputeImage::acquireOwnership(
    vkw::CommandBuffer &buffer, uint32_t incomingFamilyIndex,
...
void TestApp::PrecomputeImage::releaseOwnershipTo(
    vkw::CommandBuffer &buffer, uint32_t computeFamilyIndex,
    VkImageLayout incomingLayout, VkAccessFlags incomingAccessMask,
    VkPipelineStageFlags incomingStageMask) const {
  VkImageMemoryBarrier transitLayout1{};
  transitLayout1.image = m_image.get();
...
void TestApp::PrecomputeImage::releaseOwnership(
    vkw::CommandBuffer &buffer, uint32_t acquireFamilyIndex,
...
void TestApp::PrecomputeImage::acquireOwnershipFrom(
    vkw::CommandBuffer &buffer, uint32_t computeFamilyIndex,
    VkImageLayout acquireLayout, VkAccessFlags acquireAccessMask,
    VkPipelineStageFlags acquireStageMask) const {
  VkImageMemoryBarrier transitLayout1{};
...
void TestApp::PrecomputeImage::dispatch(vkw::CommandBuffer &buffer) {
  RenderEngine::Compute::dispatch(buffer, m_cached_group_size.first,
                                  m_cached_group_size.second);
}
