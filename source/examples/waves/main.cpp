...
public:
  WavesApp()
      : CommonApp(AppCreateInfo{
            ...
            [](vkw::PhysicalDevice &device) {
              ...
              auto dedicatedComputeFamily = std::find_if(
                  device.queueFamilies().begin(), device.queueFamilies().end(),
                  [](auto &family) {
                    return family.strictly(TestApp::dedicatedCompute());
                  });

              if (dedicatedComputeFamily == device.queueFamilies().end())
                throw std::runtime_error(
                    "Device does not have dedicated compute queue family");
        ...
        waveMaterialWireframe(device(), shaderLoader(), waveSurfaceTexture,
                              true),
        land(device(), shaderLoader()), landMaterial(device(), shaderLoader()),
        ...
        computeSubmitInfo(computeCommandBuffer, *computeImageRelease,
                          VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                          *computeImageReady),
        fence(std::make_shared<vkw::Fence>(device())),
        waveSettings(
            gui(), waves, waveSurfaceTexture,
            {{"solid", waveMaterial}, {"wireframe", waveMaterialWireframe}}),
        landSettings(
            gui(), land,
            {{"solid", landMaterial}, {"wireframe", landMaterialWireframe}}),
...

  {
    ...
    addMainPassDependency(computeImageReady,
                          VK_PIPELINE_STAGE_VERTEX_INPUT_BIT);
    signalOnMainPassComplete(computeImageRelease);

    auto syncCSubmitInfo = vkw::SubmitInfo{
        std::span<vkw::Semaphore const>{computeImageReady.get(), 0},
        {},
        std::span<vkw::Semaphore const>{computeImageReady.get(), 1}};
    ...
    waveSurfaceTexture.acquireOwnership(
        computeCommandBuffer, mainPassQueueFamilyIndex(),
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT,
        VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    waveSurfaceTexture.dispatch(computeCommandBuffer);
    waveSurfaceTexture.releaseOwnership(
        computeCommandBuffer, mainPassQueueFamilyIndex(),
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT,
...
protected:
  void preMainPass(vkw::PrimaryCommandBuffer &buffer,
    ...
    waveSurfaceTexture.acquireOwnershipFrom(
        buffer, computeCommandBuffer.queueFamily(),
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT,
        VK_PIPELINE_STAGE_VERTEX_SHADER_BIT);
  }
  ...
  void onMainPass(vkw::PrimaryCommandBuffer &buffer,
    ...
    if (landSettings.enabled()) {
      ...
      land.draw(recorder, globalState.camera().position(),
                globalState.camera());
    }

    if (waveSettings.enabled()) {
      recorder.setMaterial(waveSettings.pickedMaterial().get());
      waves.draw(recorder, globalState.camera().position(),
                 globalState.camera());
    }
  }
  void afterMainPass(vkw::PrimaryCommandBuffer &buffer,
                     RenderEngine::GraphicsPipelinePool &pool) override {
    waveSurfaceTexture.releaseOwnershipTo(
        buffer, computeCommandBuffer.queueFamily(),
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT,
        VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
  }
  ...
  void postSubmit() override { computeQueue.submit(computeSubmitInfo, *fence); }
...
int main() { return ErrorCallbackWrapper<decltype(&runWaves)>::run(&runWaves); }