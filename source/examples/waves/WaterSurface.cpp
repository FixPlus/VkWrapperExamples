...
WaterSurface::Geometry::Geometry(vkw::Device &device, WaterSurface &surface,
                                 WaveSurfaceTexture &texture)
    : RenderEngine::Geometry(surface), m_sampler(m_sampler_create(device)) {
  ...
  for (int i = 0; i < cascadeCount; ++i) {
    ...
    auto &displacement =
        m_displacements_view.emplace_back(device, cascade, cascade.format());
    // auto &derivatives = cascade.getView<vkw::ColorImageView>(device,
    // cascade.format(), 1, mapping);
    set().write(i + 1, displacement, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                m_sampler);
  }
}
...
WaterMaterial::Material::Material(vkw::Device &device,
                                  WaterMaterial &waterMaterial,
                                  WaveSurfaceTexture &texture)
    : RenderEngine::Material(waterMaterial),
      m_buffer(device,
  ...
  for (int i = 0; i < cascadeCount; ++i) {
    ...
    auto &[derivatives, turbulence] = m_derivTurbViews.emplace_back(
        vkw::ImageView<vkw::COLOR, vkw::V2D>{device, tex, tex.format(), 1},
        vkw::ImageView<vkw::COLOR, vkw::V2D>{device, tex, tex.format(), 2});
    set().write(2 + i, derivatives, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                m_sampler);
    set().write(2 + cascadeCount + i, turbulence,
                VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, m_sampler);
  }
}

WaveSettings::WaveSettings(
    TestApp::GUIFrontEnd &gui, WaterSurface &water, WaveSurfaceTexture &texture,
    std::map<std::string, vkw::StrongReference<WaterMaterial>> materials)
    : TestApp::GridSettings(gui, water, "Waves"), m_water(water),
      m_materials(std::move(materials)), m_texture(texture) {
  if (m_materials.empty())
    throw std::runtime_error(
        "Cannot create WaveSettings window with empty material map");
...
void WaveSettings::onGui() {
  ...
  if (!ImGui::CollapsingHeader("Surface waves"))
    return;
  ImGui::SliderFloat("Stretch", &m_texture.get().dynamicSpectrumParams.stretch,
                     0.0f, 10.0f);

  m_need_update_static_spectrum =
      ImGui::SliderFloat("Wind Direction",
                         &m_texture.get().spectrumParameters.angle, 0.0f,
                         2.0f * glm::pi<float>()) ||
      m_need_update_static_spectrum;
  m_need_update_static_spectrum =
      ImGui::SliderFloat(
          "Gravity", &m_texture.get().spectrumParameters.GravityAcceleration,
          0.0f, 100.0f) ||
      m_need_update_static_spectrum;

  m_need_update_static_spectrum =
      ImGui::SliderFloat("Fetch", &m_fetch, 0.0f, 100.0f) ||
      m_need_update_static_spectrum;
  m_need_update_static_spectrum =
      ImGui::SliderFloat("WindSpeed", &m_wind_speed, 0.0f, 100.0f) ||
      m_need_update_static_spectrum;
  m_need_update_static_spectrum =
      ImGui::SliderFloat("Height scale",
                         &m_texture.get().spectrumParameters.scale, 0.0f,
                         100.0f) ||
      m_need_update_static_spectrum;
  m_need_update_static_spectrum =
      ImGui::SliderFloat("Gamma", &m_texture.get().spectrumParameters.gamma,
                         0.0f, 100.0f) ||
      m_need_update_static_spectrum;
  m_need_update_static_spectrum =
      ImGui::SliderFloat("Spread blend",
                         &m_texture.get().spectrumParameters.spreadBlend, 0.0f,
                         1.0f) ||
      m_need_update_static_spectrum;
  m_need_update_static_spectrum =
      ImGui::SliderFloat("Swell", &m_texture.get().spectrumParameters.swell,
                         0.0f, 100.0f) ||
      m_need_update_static_spectrum;
#if 0
...
#endif
  m_need_update_static_spectrum =
      ImGui::SliderFloat("Short waves fade",
                         &m_texture.get().spectrumParameters.shortWavesFade,
                         0.0f, 1.0f) ||
      m_need_update_static_spectrum;
  m_need_update_static_spectrum =
      ImGui::SliderFloat("Depth", &m_texture.get().spectrumParameters.Depth,
                         1.0f, 1000.0f) ||
      m_need_update_static_spectrum;
  ImGui::Text("alpha: %.3f, peak omega %.3f",
              m_texture.get().spectrumParameters.alpha,
              m_texture.get().spectrumParameters.peakOmega);
  ...
  for (int i = 0; i < m_texture.get().cascadesCount(); ++i) {
    ...
    m_need_update_static_spectrum =
        ImGui::SliderFloat("Cutoff low", &params.CutoffLow, 0.0f, 1000.0f) ||
        m_need_update_static_spectrum;
    m_need_update_static_spectrum =
        ImGui::SliderFloat("Cutoff high", &params.CutoffHigh, 200.0f,
                           10000.0f) ||
        m_need_update_static_spectrum;
...
WaveSettings::WaveSettings(WaveSettings &&another) noexcept
    : TestApp::GridSettings(std::move(another)), m_water(another.m_water),
...
void WaveSettings::m_calculate_alpha() {
  m_texture.get().spectrumParameters.alpha =
      0.076f * glm::pow(m_texture.get().spectrumParameters.GravityAcceleration *
                            m_fetch / m_wind_speed / m_wind_speed,
                        -0.22f);
}
...
void WaveSettings::m_calculate_peak_frequency() {
  ...
  m_texture.get().spectrumParameters.peakOmega =
      22.0f * glm::pow(m_wind_speed * m_fetch / g / g, -0.33f);
}
...
WaveSurfaceTexture::WaveSurfaceTexture(
    ...
    : m_spectrum_precompute_layout(
      ...
      m_scales_buffer(
          device, VmaAllocationCreateInfo{
                      .usage = VMA_MEMORY_USAGE_CPU_TO_GPU,
                      .requiredFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT}) {
  ...
  for (int i = 0; i < cascades; ++i) {
    ...
    auto &cascade = m_cascades.emplace_back(
        *this, m_spectrum_precompute_layout, m_dyn_spectrum_gen_layout,
        m_combiner_layout, device, m_spectrum_params, m_dyn_params,
        baseCascadeSize, cascadeSizes[i]);
...
WaveSurfaceTexture::WaveSurfaceTextureCascade::WaveSurfaceTextureCascade(
    WaveSurfaceTexture &parent,
    TestApp::PrecomputeImageLayout &spectrumPrecomputeLayout,
    RenderEngine::ComputeLayout &dynSpectrumCompute,
    RenderEngine::ComputeLayout &combinerLayout, vkw::Device &device,
    ...
    : WaveSurfaceTextureCascadeImageHandler(device, cascadeSize),
      m_spectrum_textures(spectrumPrecomputeLayout, spectrumParams, device,
                          cascadeSize, cascadeScale),
      m_device(device), m_layout(spectrumPrecomputeLayout),
      m_dynamic_spectrum(dynSpectrumCompute, m_spectrum_textures, dynParams,
                         device, cascadeSize),
...
void WaveSurfaceTexture::WaveSurfaceTextureCascade::precomputeStaticSpectrum(
    vkw::CommandBuffer &buffer) {
...
void WaveSurfaceTexture::WaveSurfaceTextureCascade::computeSpectrum(
  ...
  transitLayout1.image =
      m_dynamic_spectrum.vkw::AllocatedImage::operator VkImage_T *();
  ...
  transitLayout1.srcQueueFamilyIndex =
      VK_QUEUE_FAMILY_IGNORED; // device.getGraphicsQueue()->familyIndex();
  transitLayout1.dstQueueFamilyIndex =
      VK_QUEUE_FAMILY_IGNORED; // queue->familyIndex();
  ...
  transitLayout1.dstAccessMask =
      VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT;
  transitLayout1.srcAccessMask = VK_ACCESS_MEMORY_WRITE_BIT;

  buffer.imageMemoryBarrier(VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                            VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                            {&transitLayout1, 1});
}

void WaveSurfaceTexture::WaveSurfaceTextureCascade::combineFinalTexture(
    vkw::CommandBuffer &buffer) {
  m_texture_combiner.dispatch(buffer, cascade_size / 16, cascade_size / 16, 1);
}

WaveSurfaceTexture::WaveSurfaceTextureCascadeImageHandler::
    WaveSurfaceTextureCascadeImageHandler(vkw::Device &device,
    ...
    : m_surfaceTexture{device.getAllocator(),
      ...
      m_views({vkw::ImageView<vkw::COLOR, vkw::V2D>{
                   device, m_surfaceTexture, m_surfaceTexture.format(), 0u, 1u},
  ...
  VkImageMemoryBarrier transitLayout1{};
  transitLayout1.image =
      m_surfaceTexture.vkw::AllocatedImage::operator VkImage_T *();
  transitLayout1.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
  transitLayout1.pNext = nullptr;
  ...
  transitLayout1.dstQueueFamilyIndex =
      device.anyGraphicsQueue().family().index();
...
WaveSurfaceTexture::WaveSurfaceTextureCascade::SpectrumTextures::GaussTexture::
    ...
    : vkw::Image<vkw::COLOR, vkw::I2D>(
          ...
          VmaAllocationCreateInfo{.usage = VMA_MEMORY_USAGE_GPU_ONLY,
                                  .requiredFlags =
                                      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT},
          VK_FORMAT_R32G32_SFLOAT, size, size, 1, 1, 1,
          VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT),
      m_view(device, *this, format()) {
  vkw::Buffer<glm::vec2> stageBuf{
      device, size * size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
      VmaAllocationCreateInfo{.usage = VMA_MEMORY_USAGE_CPU_TO_GPU,
                              .requiredFlags =
                                  VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT}};
  ...
  transferCommand.imageMemoryBarrier(VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                                     VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                     {&transitLayout1, 1});
  ...
  transferCommand.copyBufferToImage(
      stageBuf, *this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, {&copyRegion, 1});
...
WaveSurfaceTexture::WaveSurfaceTextureCascade::SpectrumTextures::
    SpectrumTextures(
        TestApp::PrecomputeImageLayout &spectrumPrecomputeLayout,
        vkw::UniformBuffer<SpectrumParameters> const &spectrumParams,
        vkw::Device &device, uint32_t cascadeSize, float cascadeScale)
    : vkw::Image<vkw::COLOR, vkw::I2D,
                 vkw::ARRAY>{device.getAllocator(),
                             VmaAllocationCreateInfo{
                                 .usage = VMA_MEMORY_USAGE_GPU_ONLY,
                                 .requiredFlags =
                                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT},
...
void FFT::ftt(vkw::CommandBuffer &buffer, const Complex2DTexture &input,
              const Complex2DTexture &output) {
  size_t x, y;
  x = input.image()->rawExtents().width;
  y = input.image()->rawExtents().height;

  auto key = std::pair<const Complex2DTexture *, const Complex2DTexture *>{
      &input, &output};
  auto found = m_computes.contains(key);
  auto &comp =
      found ? m_computes.at(key)
            : m_computes
                  .emplace(
                      key,
                      std::tuple<PermuteRow, FFTRow, PermuteColumn, FFTColumn>{
                          PermuteRow{m_permute_row_layout, input, output},
                          FFTRow{m_fft_row_layout, output},
                          PermuteColumn{m_permute_column_layout, output},
                          FFTColumn{m_fft_column_layout, output}})
                  .first->second;
  ...
  buffer.imageMemoryBarrier(VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                            VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                            {&transitLayout1, 1});
  transitLayout1.srcAccessMask =
      VK_ACCESS_MEMORY_WRITE_BIT | VK_ACCESS_MEMORY_READ_BIT;
  uint32_t size = 1;
  ...
  while (size <= x) {
    buffer.pushConstants(fft_row.layout().pipelineLayout(),
  ...
  while (size <= x) {
    buffer.pushConstants(fft_column.layout().pipelineLayout(),
                         VK_SHADER_STAGE_COMPUTE_BIT, 0, size);
...
FFT::PermuteRow::PermuteRow(RenderEngine::ComputeLayout &layout,
                            const Complex2DTexture &input,
                            const Complex2DTexture &output)
...
FFT::FFTRow::FFTRow(RenderEngine::ComputeLayout &layout,
                    const Complex2DTexture &io)
    : RenderEngine::Compute(layout) {
  set().writeStorageImage(0, io);
}

FFT::PermuteColumn::PermuteColumn(RenderEngine::ComputeLayout &layout,
                                  const Complex2DTexture &io)
    : RenderEngine::Compute(layout) {
  set().writeStorageImage(0, io);
}

FFT::FFTColumn::FFTColumn(RenderEngine::ComputeLayout &layout,
                          const Complex2DTexture &io)
    : RenderEngine::Compute(layout) {
  set().writeStorageImage(0, io);
}

WaveSurfaceTexture::WaveSurfaceTextureCascade::DynamicSpectrumTextures::
    DynamicSpectrumTextures(
    ...
    : vkw::Image<vkw::COLOR, vkw::I2D, vkw::ARRAY>(
          device.getAllocator(),
          VmaAllocationCreateInfo{.usage = VMA_MEMORY_USAGE_GPU_ONLY,
                                  .requiredFlags =
                                      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT},
          VK_FORMAT_R32G32_SFLOAT, cascadeSize, cascadeSize, 1, 8, 1,
          VK_IMAGE_USAGE_STORAGE_BIT),
      RenderEngine::Compute(layout),
...
WaveSurfaceTexture::WaveSurfaceTextureCascade::FinalTextureCombiner::
    FinalTextureCombiner(RenderEngine::ComputeLayout &layout,
                         DynamicSpectrumTextures &specTex,
